		...
		for filename in listdir:
			...
			if len(arr) > 0:
				...
				if arr_var[0,1]>1 and arr_var[0,2]>5 and arr_var[0,3]>0.5: #坏值阈值
					...
					if filename[10:12] == '03': arr3=np.vstack((arr3,arr))
					if filename[10:12] == '04': arr4=np.vstack((arr4,arr))
					if filename[10:12] == '05': arr5=np.vstack((arr5,arr))
					if filename[10:12] == '06': arr6=np.vstack((arr6,arr))
					if filename[10:12] == '07': arr7=np.vstack((arr7,arr))
					if filename[10:12] == '08': arr8=np.vstack((arr8,arr))
					if filename[10:12] == '09': arr9=np.vstack((arr9,arr))
					if filename[10:12] == '10': arr10=np.vstack((arr10,arr))
					if filename[10:12] == '11': arr11=np.vstack((arr11,arr))
					if filename[10:12] == '12': arr12=np.vstack((arr12,arr))
	arr = [arr1,arr2,arr3,arr4,arr5,arr6,arr7,arr8,arr9,arr10,arr11,arr12]
	arr_months = np.empty((0,10),dtype=object)
	for i in range(0,12,1):
		arr_month = np.array([i+1,np.max(arr[i][:,1]),np.median(arr[i][:,1]),np.min(arr[i][:,1]), np.max(arr[i][:,2]),np.median(arr[i][:,2]),np.min(arr[i][:,2]), np.max(arr[i][:,3]),np.median(arr[i][:,3]),np.min(arr[i][:,3])])
		arr_months = np.vstack((arr_months, arr_month))
	np.savetxt(path+'arr_months.dat', arr_months, fmt="%.1f", delimiter='   ')
#prodata(path)

def plomonths(path,file):
	months=[]; dataset=pd.read_csv('../rawdata/arr_months.dat',header=None,sep=' +',engine='python')
	months.extend(dataset.values[:,0]); arr_months = np.zeros((len(months),10)) #单独以 months 为 list 是因为定义 arr 要用到
	arr_months[:,0]=dataset.values[:,0]
	arr_months[:,1]=dataset.values[:,1]; arr_months[:,2]=dataset.values[:,2]; arr_months[:,3]=dataset.values[:,3]
	arr_months[:,4]=dataset.values[:,4]; arr_months[:,5]=dataset.values[:,5]; arr_months[:,6]=dataset.values[:,6]
	arr_months[:,7]=dataset.values[:,7]; arr_months[:,8]=dataset.values[:,8]; arr_months[:,9]=dataset.values[:,9]
	arr_months[:,1]=arr_months[:,1]-273.15; arr_months[:,2]=arr_months[:,2]-273.15; arr_months[:,3]=arr_months[:,3]-273.15
	print(np.max(arr_months[:,1]), np.median(arr_months[:,2]), np.min(arr_months[:,3]));
	print(np.max(arr_months[:,4]), np.median(arr_months[:,5]), np.min(arr_months[:,6]));
	print(np.max(arr_months[:,7]), np.median(arr_months[:,8]), np.min(arr_months[:,9]));
	...
	for col in range(1,len(arr_months[0,:]),1):
		if col < 4:
			plt.subplot(131)
			plt.plot(arr_months[:,0],arr_months[:,col],linestyle='-', marker='o',label=label[col]); plt.legend(loc='upper right')
			plt.xticks(arr_months[:,0],rotation=60); plt.grid(color='cyan', linestyle='--') #蓝绿色
			plt.title('Temperature variation', fontproperties=myfont)
		elif col < 7:
			plt.subplot(132)
			plt.plot(arr_months[:,0],arr_months[:,col],linestyle='-', marker='o',label=label[col]); plt.legend(loc='upper right')
			plt.xticks(arr_months[:,0],rotation=60);plt.grid(color='cyan', linestyle='--')
	...
	plt.savefig(path+'arr_months.png')
prodata(path)
plomonths(path,'arr_months.dat')